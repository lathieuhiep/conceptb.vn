(function () {
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var certificationSlider = document.querySelector('[data-about-certification-slider]');

        if (certificationSlider && typeof window.Swiper === 'function') {
            var certificationSwiper = certificationSlider.querySelector('[data-about-certification-swiper]');
            var certificationSlideCount = certificationSwiper ? certificationSwiper.querySelectorAll('.swiper-slide').length : 0;

            if (certificationSwiper && !certificationSwiper.classList.contains('swiper-initialized')) {
                new window.Swiper(certificationSwiper, {
                    slidesPerView: 1,
                    spaceBetween: 12,
                    loop: false,
                    rewind: certificationSlideCount > 4,
                    speed: 500,
                    watchOverflow: true,
                    navigation: {
                        prevEl: certificationSlider.querySelector('.about-certification__nav--prev'),
                        nextEl: certificationSlider.querySelector('.about-certification__nav--next'),
                    },
                    breakpoints: {
                        375: {
                            slidesPerView: 2,
